...
import type { Language, ThemeId, ProviderInfo, ProviderRef } from "../types";
...
type Tab = "general" | "gameplay" | "providers" | "profiles";
...
export default function SettingsPage() {
  ...
  }, [s.customEndpoints.length, s.apiKeys.openai, s.apiKeys.gemini]);
  ...
  const TABS: { id: Tab; label: string; icon: string }[] = [
    ...
    { id: "gameplay", label: t("settings.tabGameplay"), icon: "♟️" },
    { id: "providers", label: t("settings.tabProviders"), icon: "🔌" },
    { id: "profiles", label: t("settings.tabProfiles"), icon: "🎭" },
  ];
        ...
        <section className="settings-section">
          ...
          <div className="settings-row">
            ...
            <select value={s.engineLevel} onChange={(e) => s.setEngineLevel(Number(e.target.value))}>
              {ENGINE_LEVELS.map((l) => <option key={l.id} value={l.id}>{l.label} (~{l.elo})</option>)}
            </select>
          </div>
          <div className="settings-row">
            <label>{t("settings.autoAnalyze")}</label>
            <input type="checkbox" checked={s.autoAnalyze} onChange={(e) => s.setAutoAnalyze(e.target.checked)} className="chk" />
          </div>
          <div className="settings-row">
        ...
        <>
          <section className="settings-section">
            ...
            {ollama !== "checking" && !ollama.ok && (
              <div className="settings-row"><label style={{ minWidth: 160 }} />
                <div className="muted-small" style={{ color: "var(--danger)" }}>⚠ {ollama.error}</div></div>
            )}
            {ollama !== "checking" && ollama.ok && ollama.models.length > 0 && (
              <div className="settings-row"><label style={{ minWidth: 160 }}>{t("settings.ollamaModels")}</label>
                <div className="muted-small">{ollama.models.join(", ")}</div></div>
            )}
          </section>
      ...
      {tab === "profiles" && (
        <section className="settings-section">
          ...
          {s.profiles.map((p) => {
            ...
            return (
              <div className={`profile-card2 ${open ? "open" : ""}`} key={p.id}>
                <div className="profile-card2-head" onClick={() => setExpandedProfile(open ? null : p.id)}>
                  <Avatar emoji={p.emoji || "🤖"} color={p.color} size={36} />
                  <div className="profile-card2-info">
                    <div className="profile-card2-name">
                      {p.name || "—"} {p.builtin && <span className="badge off">{t("profiles.builtin")}</span>}
                    </div>
                    <div className="muted-small">{p.rating} · {providerDisplayName(p.provider, contextFromSettings(s))} · {p.model}</div>
                  </div>
                  <button className="danger" onClick={(e) => { e.stopPropagation(); s.removeProfile(p.id); }}>{t("profiles.delete")}</button>
                  <span className="chev">{open ? "▴" : "▾"}</span>
                </div>
                {open && (
                  <div className="profile-card2-body">
                    <div className="profile-editor-grid">
                      <div><label>{t("profiles.name")}</label>
                        <input value={p.name} onChange={(e) => s.updateProfile(p.id, { name: e.target.value })} /></div>
                      <div><label>{t("profiles.emoji")}</label>
                        <input className="emoji-input" value={p.emoji} maxLength={4} onChange={(e) => s.updateProfile(p.id, { emoji: e.target.value })} /></div>
                      <div><label>{t("profiles.color")}</label>
                        <input type="color" value={p.color} onChange={(e) => s.updateProfile(p.id, { color: e.target.value })} style={{ height: 38, padding: 2 }} /></div>
                      <div><label>{t("profiles.rating")}</label>
                        <input type="number" value={p.rating} onChange={(e) => s.updateProfile(p.id, { rating: Number(e.target.value) })} /></div>
                      <div><label>{t("profiles.provider")}</label>
                      ...
                      <div><label>{t("profiles.model")}</label>
                    ...
                    {!isEngine && (
                      <div><label>{t("profiles.prompt")}</label>
                        <textarea value={p.systemPrompt} placeholder={t("profiles.promptPlaceholder")}
                          onChange={(e) => s.updateProfile(p.id, { systemPrompt: e.target.value })} /></div>
                    )}
                  </div>
                )}
              </div>
          ...
          <button className="primary" onClick={() => { const id = s.addProfile(); setExpandedProfile(id); }} style={{ marginTop: 10 }}>
            {t("profiles.add")}
          </button>
        </section>
      )}
